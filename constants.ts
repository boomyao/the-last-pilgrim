...
// Import audio durations data
// This will be generated by running: npm run calculate-durations
let audioDurationsData: any = null;
try {
  audioDurationsData = require('./audioDurations.json');
} catch (error) {
  // File doesn't exist yet, will use fallback values
  console.warn('audioDurations.json not found. Run "npm run calculate-durations" to generate it.');
}
...
// Get the audio durations data, with fallback if file doesn't exist
let audioDurations: AudioDurationsData | null = null;
try {
  audioDurations = audioDurationsData as AudioDurationsData;
} catch (error) {
  console.warn('Could not load audioDurations.json. Using fallback durations.');
}

// Get the duration of a specific track
export const getTrackDuration = (audioFile: string): number => {
  if (audioDurations && audioDurations.trackDurations[audioFile]) {
...
export const getTotalDuration = (): number => {
  ...
  return CHAPTERS.reduce((total, chapter, chapterIndex) => {
